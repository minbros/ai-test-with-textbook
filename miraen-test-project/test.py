...
#김찬우 api key
#client = OpenAI(api_key="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")
...
for i in range(6):
    vectors_of_explains.append(explains[i])


for i in range(6):
    index.upsert(
        vectors=[{"id": english_subtitles[i], "values": embed_text(vectors_of_explains[i])}]
    )
...
sample_explain = "Add polynomials of the same degree together."
